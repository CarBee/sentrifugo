...
- `Zend_*` class references in `application/`: **4881**
...
4. Migrated `Default_Plugin_SecurityCheck` to use Laminas ACL type hints and native JSON encoding, and replaced direct `Zend_Auth_Storage_Session` instantiation with auth storage access via `Zend_Auth::getInstance()->getStorage()`.
5. Preserved runtime behavior while reducing coupling to direct Zend file paths.
